...
export default function AdminPayments() {
  ...
  return (
    <div style={{ maxWidth: 640 }}>
      <h1 style={{ fontSize: 24, marginBottom: 4 }}>Pago por transferencia</h1>
      <p style={{ color: 'var(--bark-3)', fontSize: 13.5, marginBottom: 20, lineHeight: 1.7 }}>
        Estos datos bancarios se envían al cliente en el correo de confirmación de su pedido.
        Paga por transferencia y el pedido se envía cuando se recibe la transferencia.
      </p>
      ...
      {loading ? (
        ...
        <form onSubmit={save} className="card" style={{ padding: 'clamp(20px, 3vw, 28px)', display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <Field label="Concepto que debe indicar el cliente *">
            ...
            <p style={{ fontSize: 11.5, color: 'var(--bark-3)', marginTop: 5 }}>
              El concepto permite asociar la transferencia al pedido correcto. Variables disponibles:{' '}
              <code style={{ background: 'var(--sand)', padding: '2px 6px', borderRadius: 6 }}>{'{num}'}</code> n.º de pedido,{' '}
              <code style={{ background: 'var(--sand)', padding: '2px 6px', borderRadius: 6 }}>{'{nom}'}</code> nombre del cliente,{' '}
              <code style={{ background: 'var(--sand)', padding: '2px 6px', borderRadius: 6 }}>{'{produit}'}</code> producto pedido.
            </p>
          </Field>
          ...
          {saved && (
            <div style={{
              ...
              <CheckIcon size={14} /> Datos bancarios guardados. Se enviarán en los próximos correos de confirmación.
            </div>
          )}
        </form>
      )}
