...
export default function AdminProducts() {
  ...
  const submit = async e => {
    ...
    try {
      ...
      setSavedMsg(editing ? 'Producto actualizado.' : 'Producto añadido.');
      closeForm();
      load();
    } catch (err) {
    ...
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', gap: 16, flexWrap: 'wrap', marginBottom: 20 }}>
        <div>
          ...
          <p style={{ color: 'var(--bark-3)', fontSize: 13.5 }}>{products.length} producto(s)</p>
        </div>
        <button className="btn-primary" onClick={openNew}>+ Añadir un producto</button>
      </div>
      ...
      {(showForm) && (
        <form onSubmit={submit} className="card" style={{ padding: 20, marginBottom: 20 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
            <h2 style={{ fontSize: 16 }}>{editing ? `Editar #${editing.id}` : 'Nuevo producto'}</h2>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '220px 1fr 1fr 1fr 1fr', gap: 12 }} className="admin-form-grid">
            ...
            <AdminField label="Nombre *"><input className="input-luxury" style={{ width: '100%' }} value={form.name} onChange={e => set({ name: e.target.value })} required /></AdminField>
            ...
            <AdminField label="Precio (€) *"><input className="input-luxury" style={{ width: '100%' }} type="number" step="0.01" min="0" value={form.price} onChange={e => set({ price: e.target.value })} required /></AdminField>
            <AdminField label="Precio anterior (€)"><input className="input-luxury" style={{ width: '100%' }} type="number" step="0.01" min="0" value={form.oldPrice} onChange={e => set({ oldPrice: e.target.value })} /></AdminField>
            <AdminField label="Distintivo">
            ...
            <AdminField label="Marca (brand)"><input className="input-luxury" style={{ width: '100%' }} value={form.brand} onChange={e => set({ brand: e.target.value })} /></AdminField>
            ...
            <AdminField label="MPN (ref. del fabricante)"><input className="input-luxury" style={{ width: '100%' }} value={form.mpn} onChange={e => set({ mpn: e.target.value })} /></AdminField>

            <AdminField label="Descripción" wide>
              <textarea className="input-luxury" rows={3} style={{ width: '100%' }} value={form.description} onChange={e => set({ description: e.target.value })} />
            </AdminField>
            <AdminField label="Puntos fuertes (1 por línea)" wide>
            ...
            <AdminField label="Especificaciones (una por línea · Formato: Etiqueta: Valor)" wide>
          ...
          <div style={{ display: 'flex', gap: 10, marginTop: 16 }}>
            <button type="submit" className="btn-primary" disabled={saving}>
              {saving ? 'Guardando…' : editing ? 'Guardar' : 'Crear el producto'}
            </button>
            <button type="button" onClick={closeForm}
            ...
            {products.map(p => (
              <tr key={p.id} style={{ borderBottom: '1px solid var(--border)' }}>
                ...
                <td style={{ padding: '10px 8px', whiteSpace: 'nowrap', textAlign: 'right' }}>
                  <button onClick={() => openEdit(p)} className="btn-ghost" style={{ padding: '8px 14px', fontSize: 12 }}>Editar</button>
